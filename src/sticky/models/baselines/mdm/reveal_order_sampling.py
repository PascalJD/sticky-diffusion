...
from sticky.models.common.reveal_order_sampling import (
    clamp_known_tokens,
...
def _normalize_sampler_method(method: str | None) -> str:
    ...
    if key not in {"uniform", "top_probability", "top_prob_margin"}:
        raise ValueError(
            f"Unknown MDM packed-sequence sampler method={method!r}. "
...
def _apply_structural_token_constraints(
    ...
    *,
    ...
    Packed content tokens must stay in the original content range. During packed
    ...
    sep_token_id = int(log_probs.shape[-1]) - 2
    eos_token_id = int(log_probs.shape[-1]) - 1
    sep_forbidden = (token_ids == sep_token_id) & response_positions[..., None]
    constrained = jnp.where(sep_forbidden, neg_inf, constrained)

    eos_forbidden = (token_ids == eos_token_id) & response_content_positions[..., None]
...
def _zero_sampling_info(dtype: Any = jnp.float32) -> dict[str, Array]:
    ...
    return {
        ...
        # Backward-compatible alias kept for older metric consumers.
...
def reveal_order_sample_step(
    ...
    *,
    ...
    eos_token_id = int(log_probs.shape[-1]) - 1
    proposal = jnp.where(eos_positions, eos_token_id, proposal)
