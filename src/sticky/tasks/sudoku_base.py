...
from sticky.data.sudoku import (
    get_sudoku_board_num_examples as get_sudoku_num_examples,
    make_sudoku_board_iterator as make_sudoku_iterator,
)
...
class SudokuTaskBase(Task):
    """Legacy compatibility shim for older Sudoku task classes.

    The canonical Sudoku benchmark in this repo is the board-level inpainting
    path. Older packed-sequence task modules still import this base, so keep the
    import surface live but route it through the board-level dataset helpers.
    """
